...
function restoreAudio() {
  console.log("Restoring Audio defaults...");
  try {
    // 1. Find a physical sink to revert to
    const sinkOutput = output("pactl list short sinks");
    const sinks = sinkOutput.split('\n').map(line => line.split('\t')[1]).filter(Boolean);

    // Find first PCI or USB sink that is NOT our virtual modules
    const physicalSink = sinks.find(s =>
      (s.includes("pci") || s.includes("usb") || s.includes("alsa_output")) &&
      !s.includes("SystemAudioSink") &&
      !s.includes("RecordingMixSink")
    );

    if (physicalSink) {
      console.log(`Reverting default sink to: ${physicalSink}`);
      run(`pactl set-default-sink ${physicalSink}`);
    } else {
      console.warn("No physical sink found to revert to. Leaving as is.");
    }

    // 2. Unload our modules
    softCleanup();
...
module.exports = { initAudio, getCurrentMode, checkAudioChanges, restoreAudio };
